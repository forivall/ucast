...
describe('Condition Interpreter', () => {
  describe('eq', () => {
    ...
    it('returns true if condition value is null and field value is undefined', () => {
      const condition = new Field('eq', 'value', null)

      expect(interpret(condition, {})).to.be.true
      expect(interpret(condition, { value: undefined })).to.be.true
    })
  })
  ...
  describe('where', () => {
    ...
    it('binds passed in object as "this"', () => {
      const object = {}
      const test = spy(function matches(this: unknown) {
        return this === object
      })
  ...
  describe('within (alias to `in`)', () => {
    const interpret = createJsInterpreter({ within })

    includeExamplesForFieldCondition('within', [])
    ...
    it('can check value of nested property', () => {
      const condition = new Field('within', 'some.age', [1, 2])
    ...
    it('can check value itself', () => {
      const condition = new Field('within', ITSELF, [1, 2])
